...
class Space(object):
    ...
    def append_by_file(self, file_path):
        ...
        for i in range(1, len(self.vertex_list_polygon_list)):
            temp = [0, 0, 0]
            length = len(self.vertex_list_polygon_list[i])
            for j in range(0, length):
                temp += self.polygon_list_normal_vector[self.vertex_list_polygon_list[i][j]]
            vertex_normal_vector = temp / length
        ...
        for i in range(1, len(self.vertex_list_coordinate)):
            self.vertex_list_coordinate[i] = transform.v3_to_mr4c1(self.vertex_list_coordinate[i])
            self.vertex_list_normal_vector[i] = transform.v3_to_mr4c1(self.vertex_list_normal_vector[i], True)
        for i in range(1, len(self.polygon_list_normal_vector)):
            self.polygon_list_normal_vector[i] = transform.v3_to_mr4c1(self.polygon_list_normal_vector[i], True)
