...
def calculate(camera, light, material, N_input, O_d_input=None):
    ...
    if O_d_input is not None:
        if type(O_d_input) != list or len(O_d_input) != 3 \
                or type(O_d_input[0]) != int or type(O_d_input[0]) != float or O_d_input[0] < 0 or O_d_input[0] > 1 \
                or type(O_d_input[1]) != int or type(O_d_input[1]) != float or O_d_input[1] < 0 or O_d_input[1] > 1 \
                or type(O_d_input[2]) != int or type(O_d_input[2]) != float or O_d_input[2] < 0 or O_d_input[2] > 1:
